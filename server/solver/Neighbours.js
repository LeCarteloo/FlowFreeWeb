...
module.exports = class Neighbours {
  ...
  static isValid(mapState, y, x, color) {
    ...
    if (
      x < GameMap.size - 1 &&
      map[y][x + 1] == GameMap.foundColors[color] &&
      mapState.current[color].X != x + 1 &&
      mapState.current[color].Y != y
    ) {
      // && x != pt.X + 1 && y != pt.Y
      return false;
    }
    ...
    if (
      y < GameMap.size - 1 &&
      map[y + 1][x] == GameMap.foundColors[color] &&
      mapState.current[color].X != x &&
      mapState.current[color].Y != y + 1
    ) {
      // && x != pt.X && y != pt.Y + 1
      return false;
    }
