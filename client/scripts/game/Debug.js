...
class Debug {
  static gameDebugInfo(event, isOn) {
    ...
    current.innerHTML = `True position of tile (Y, X):  ${
      Math.floor(event.offsetX / tileW) * tileW
    } 
    ...
    for (var y = 0; y < mapLength; y++) {
      for (var x = 0; x < mapLength; x++) {
        gameMapArray.innerHTML += `<p style="display: inline-block; font-weight: bold; color:${
          Colors[gameMap[y][x].toUpperCase()]
        }">
                 ${gameMap[y][x]} </p>`;
      }
